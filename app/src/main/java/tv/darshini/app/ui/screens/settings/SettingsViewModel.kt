...
import tv.darshini.app.BuildConfig
...
import tv.darshini.app.update.AppUpdateDownloadState
import tv.darshini.app.update.AppUpdateDownloadStatus
import tv.darshini.app.update.AppUpdateInstaller
import tv.darshini.app.update.GitHubReleaseChecker
import tv.darshini.app.update.GitHubReleaseInfo
...
import tv.darshini.domain.manager.BackupConflictStrategy
import tv.darshini.domain.manager.BackupImportPlan
import tv.darshini.domain.manager.BackupManager
...
import tv.darshini.domain.model.LiveStreamFormatMode
...
import tv.darshini.domain.usecase.ExportBackup
import tv.darshini.domain.usecase.ExportBackupCommand
import tv.darshini.domain.usecase.ExportBackupResult
import tv.darshini.domain.usecase.ImportBackup
import tv.darshini.domain.usecase.ImportBackupCommand
import tv.darshini.domain.usecase.ImportBackupResult
import tv.darshini.domain.usecase.InspectBackupCommand
import tv.darshini.domain.usecase.InspectBackupResult
...
import tv.darshini.domain.usecase.SyncProviderCommand
import tv.darshini.domain.usecase.SyncProviderResult
import tv.darshini.player.AudioCompatibilityMemoryStore
...
class SettingsViewModel @Inject constructor(
    ...
    private val playbackHistoryRepository: tv.darshini.domain.repository.PlaybackHistoryRepository,
    ...
    private val epgSourceRepository: tv.darshini.domain.repository.EpgSourceRepository,
    ...
    private fun tv.darshini.app.diagnostics.CrashReportSummary.toUiModel(): CrashReportUiModel =
        CrashReportUiModel(
            timestamp = timestamp,
            exception = exception,
            fileName = fileName,
    ...
    private fun registerPreferenceObservers() {
        ...
        viewModelScope.launch {
            activeProviderIdFlow.flatMapLatest { providerId ->
                if (providerId != null) {
                    combine(
                        ...
                        val liveCats = arrays[0] as List<Category>
                        val movieCats = arrays[1] as List<Category>
                        val seriesCats = arrays[2] as List<Category>
                        val hiddenLive = arrays[3] as Set<Long>
                        val hiddenMovie = arrays[4] as Set<Long>
                        val hiddenSeries = arrays[5] as Set<Long>
                        CategoryDataWrapper(
                            liveCategories = liveCats.filter { it.id != ChannelRepository.ALL_CHANNELS_ID },
                            movieCategories = movieCats.filter { it.id != ChannelRepository.ALL_CHANNELS_ID },
                            seriesCategories = seriesCats.filter { it.id != ChannelRepository.ALL_CHANNELS_ID },
                    ...
                    flowOf(CategoryDataWrapper())
                }
            }.collect { data ->
                _uiState.update { state ->
                    state.copy(
                        availableLiveCategories = data.liveCategories,
                        availableMovieCategories = data.movieCategories,
                        availableSeriesCategories = data.seriesCategories,
                        hiddenCategoryIds = mapOf(
                            ContentType.LIVE to data.hiddenLive,
                            ContentType.MOVIE to data.hiddenMovie,
                            ContentType.SERIES to data.hiddenSeries
                        )
                    )
                }
            }
    ...
    fun toggleCategoryHidden(contentType: ContentType, categoryId: Long) {
        val providerId = _uiState.value.activeProviderId ?: return
        viewModelScope.launch {
            val currentHidden = _uiState.value.hiddenCategoryIds[contentType] ?: emptySet()
            val updated = if (currentHidden.contains(categoryId)) {
                currentHidden - categoryId
            } else {
                currentHidden + categoryId
            }
            preferencesRepository.setHiddenCategoryIds(providerId, contentType, updated)
        }
    }

    private data class CategoryDataWrapper(
        ...
        val hiddenLive: Set<Long> = emptySet(),
        val hiddenMovie: Set<Long> = emptySet(),
        val hiddenSeries: Set<Long> = emptySet()
    )
    ...
    fun setCategoryLanguagePriority(languages: List<String>) {
        viewModelScope.launch {
    ...
    fun setUseSideNavigation(enabled: Boolean) {
        viewModelScope.launch {
            preferencesRepository.setUseSideNavigation(enabled)
        }
    }
    ...
    fun setProviderSyncCadence(cadence: SyncCadence) {
        viewModelScope.launch {
            preferencesRepository.setProviderSyncCadence(cadence)
        }
    }
    ...
    fun setPlayerSurfaceMode(mode: tv.darshini.domain.model.PlayerSurfaceMode) {
        viewModelScope.launch {
    ...
    fun exportProvider(providerId: Long, uriString: String, onSuccess: (() -> Unit)? = null) {
        viewModelScope.launch {
            _uiState.update { it.copy(isSyncing = true) }
            val result = backupManager.exportProvider(providerId, uriString)
            _uiState.update { state ->
                state.copy(
                    isSyncing = false,
