...
internal fun SettingsScreenDialogs(
    ...
    dialogState.showCategoryLanguagePriorityDialogType?.let { typeName ->
        val contentType = ContentType.entries.firstOrNull { it.name == typeName }
        if (contentType != null) {
            val categories = when (contentType) {
                ContentType.LIVE -> uiState.availableLiveCategories
                ContentType.MOVIE -> uiState.availableMovieCategories
                ContentType.SERIES -> uiState.availableSeriesCategories
                else -> emptyList()
            }
            val hiddenIds = uiState.hiddenCategoryIds[contentType] ?: emptySet()
            val title = when (contentType) {
                ContentType.LIVE -> "Live TV Custom Order"
                ContentType.MOVIE -> "Movie Custom Order"
            ...
            CategoryLanguagePriorityDialog(
                ...
                onToggleVisibility = { categoryId ->
                    viewModel.toggleCategoryHidden(contentType, categoryId)
                },
                            ...
                            Text(
                                text = stringResource(
                                    when (mode) {
                                        ...
                                        ExternalPlaybackMode.ASK_EVERY_TIME -> R.string.settings_external_playback_mode_ask_every_time
                                    }
                                ),
