...
import tv.darshini.app.ui.theme.AccentGreen
    ...
    item {
        ...
        TvClickableSurface(
            ...
            Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                    Text(text = stringResource(R.string.settings_use_side_navigation), style = MaterialTheme.typography.bodyMedium, color = OnSurface)
                    Text(text = stringResource(R.string.settings_use_side_navigation_subtitle), style = MaterialTheme.typography.bodySmall, color = OnBackground.copy(alpha = 0.6f))
                }
                Switch(checked = uiState.useSideNavigation, onCheckedChange = { viewModel.setUseSideNavigation(it) })
            }
        }
        TvClickableSurface(
