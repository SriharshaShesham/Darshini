package tv.darshini.app.ui.screens.home
...
import tv.darshini.app.ui.components.SearchInput
import tv.darshini.app.ui.components.SelectionChip
import tv.darshini.app.ui.components.SelectionChipRow
...
import tv.darshini.app.ui.components.CategoryRow
import tv.darshini.app.ui.components.ChannelCard
import tv.darshini.app.ui.components.LiveSourceSwitcher
import tv.darshini.app.ui.components.shell.ContentMetadataStrip
import tv.darshini.app.ui.components.shell.LiveChannelRowSurface
import tv.darshini.app.ui.components.shell.StatusPill
import tv.darshini.app.ui.components.TvEmptyState
import tv.darshini.app.ui.components.dialogs.CategoryOptionsDialog
import tv.darshini.app.ui.components.dialogs.HiddenCategoriesDialog
import tv.darshini.app.ui.components.dialogs.HiddenChannelsDialog
import tv.darshini.app.ui.components.dialogs.PinDialog
import tv.darshini.app.ui.components.dialogs.PremiumDialog
import tv.darshini.app.ui.components.dialogs.PremiumDialogActionButton
import tv.darshini.app.ui.components.dialogs.PremiumDialogFooterButton
import tv.darshini.app.ui.components.dialogs.RenameGroupDialog
import tv.darshini.app.ui.components.ReorderTopBar
...
import tv.darshini.app.ui.screens.multiview.MultiViewViewModel
import tv.darshini.app.ui.screens.multiview.MultiViewPlannerDialog
import tv.darshini.app.navigation.Routes
import tv.darshini.domain.model.VirtualCategoryIds
import tv.darshini.domain.repository.ChannelRepository
import androidx.compose.ui.viewinterop.AndroidView
import tv.darshini.player.PlayerSurfaceResizeMode
import java.text.SimpleDateFormat
import java.util.Date
import java.util.Locale
import tv.darshini.app.ui.interaction.TvClickableSurface
import tv.darshini.app.ui.interaction.TvButton
import tv.darshini.app.ui.remote.LiveBrowseRemoteShortcutHandler
import tv.darshini.app.ui.remote.dispatchLiveBrowseRemoteShortcut
import tv.darshini.app.ui.remote.remoteColorButtonForKeyCode
...
fun HomeScreen(
    ...
    android.util.Log.e("HomeScreenFocus", "HomeScreen composable invoked!")
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        AppScreenScaffold(
            ...
            android.util.Log.e("HomeScreenFocus", "Content branch: allProviders=${uiState.allProviders.size} isCategoriesLoading=${uiState.isCategoriesLoading} cats=${uiState.categories.size}")
            if (uiState.allProviders.isEmpty() && !uiState.isCategoriesLoading) {
                ...
                val focusRestoreEnabled = !hasOverlay && !uiState.isLoading && uiState.categories.isNotEmpty()
                LaunchedEffect(focusRestoreEnabled) {
                    android.util.Log.d("HomeScreenFocus", "FocusRestoreHost enabled=$focusRestoreEnabled (hasOverlay=$hasOverlay isLoading=${uiState.isLoading} cats=${uiState.categories.size} channels=${uiState.filteredChannels.size} hasChannels=${uiState.hasChannels})")
                }
                FocusRestoreHost(
                    enabled = focusRestoreEnabled,
                    onRestore = {
                        // Try to restore last focused channel if it's still visible in the list
                        val targetChannelId = lastFocusedChannelId
                            ?.takeIf { id -> uiState.filteredChannels.any { it.id == id } }

                        android.util.Log.d("HomeScreenFocus", "onRestore fired: filteredChannels=${uiState.filteredChannels.size} hasChannels=${uiState.hasChannels} isLoading=${uiState.isLoading} lastFocused=$lastFocusedChannelId targetChannelId=$targetChannelId categoryFocuserCount=${categoryFocusRequesters.size} channelFocuserCount=${channelFocusRequesters.size}")

                        var focused = false
                        for (delayMs in longArrayOf(80L, 120L, 200L)) {
                            kotlinx.coroutines.delay(delayMs)
                            android.util.Log.d("HomeScreenFocus", "onRestore attempt after ${delayMs}ms: filteredChannels=${uiState.filteredChannels.size} channelFocusers=${channelFocusRequesters.size} targetChannelId=$targetChannelId")
                            focused = if (targetChannelId != null && channelFocusRequesters.containsKey(targetChannelId)) {
                                // Try the saved channel; if detached (scrolled off), fall through to first channel
                                val restored = channelFocusRequesters[targetChannelId]
                                    ?.requestFocusSafely(tag = "HomeScreenFocus", target = "Restore channel") ?: false
                                android.util.Log.d("HomeScreenFocus", "restoreChannel result=$restored")
                                if (restored) true
                                else initialContentFocusRequester
                                    .requestFocusSafely(tag = "HomeScreenFocus", target = "Initial content fallback")
                                    .also { android.util.Log.d("HomeScreenFocus", "initialContent fallback result=$it") }
                            } else {
                                initialContentFocusRequester
                                    .requestFocusSafely(tag = "HomeScreenFocus", target = "Initial content")
                                    .also { android.util.Log.d("HomeScreenFocus", "initialContent result=$it") }
                            }
                            if (focused) break
                        }
                        // Category fallback if channel focus failed
                        if (!focused) {
                            android.util.Log.d("HomeScreenFocus", "All channel retries failed, trying category fallback. visibleCategories=${visibleCategories.size} unlockedVisible=${unlockedVisibleCategories.size} categoryFocusers=${categoryFocusRequesters.size}")
                            val fallbackCategoryId = (unlockedVisibleCategories.firstOrNull()
                                ?: visibleCategories.firstOrNull())?.id
                            if (fallbackCategoryId != null) {
                                kotlinx.coroutines.delay(80)
                                val catResult = categoryFocusRequesters[fallbackCategoryId]
                                    ?.requestFocusSafely(tag = "HomeScreenFocus", target = "Fallback category") ?: false
                                android.util.Log.d("HomeScreenFocus", "categoryFallback id=$fallbackCategoryId inMap=${categoryFocusRequesters.containsKey(fallbackCategoryId)} result=$catResult")
                            } else {
                                android.util.Log.d("HomeScreenFocus", "No category available for fallback!")
                            }
                        } else {
                            android.util.Log.d("HomeScreenFocus", "Focus successfully set!")
                        }
                    }
                ) {
                Row(
                            ...
                            val handler = when (val target = focusedRemoteShortcutTarget) {
                                ...
                                is FocusedRemoteShortcutTarget.CategoryTarget -> {
                                    ...
                                    if (isLockedCategory && action in setOf(
                                            tv.darshini.domain.model.RemoteShortcutAction.PIN_CATEGORY,
                                            tv.darshini.domain.model.RemoteShortcutAction.HIDE_CATEGORY
                                        )
                                    ) {
                                        null
                                    } else {
                ...
                Row(
                    ...
                    Column(
                        ...
                        key(uiState.selectedCategory?.id) {
                        if (uiState.isLoading) {
                            ...
                            LazyColumn(
                                ...
                                items(
                                    ...
                                    val isFirstChannel = channel.id == uiState.filteredChannels.firstOrNull()?.id
                                    if (isFirstChannel) {
                                        DisposableEffect(channel.id) {
                                            android.util.Log.d("HomeScreenFocus", "FIRST CHANNEL ${channel.id} COMPOSED (initialContentFocusRequester attaching)")
                                            onDispose { android.util.Log.d("HomeScreenFocus", "FIRST CHANNEL ${channel.id} DISPOSED (initialContentFocusRequester detaching)") }
                                        }
                                    }
                                    LiveChannelRowSurface(
                                        ...
                                        modifier = Modifier
                                            ...
                                            .then(if (isFirstChannel) Modifier.focusRequester(initialContentFocusRequester) else Modifier)
