...
import tv.darshini.app.cast.CastManager
import tv.darshini.app.cast.CastMediaRequest
import tv.darshini.app.cast.CastStartResult
import tv.darshini.app.di.MainPlayerEngine
import tv.darshini.app.player.LivePreviewHandoffManager
import tv.darshini.app.player.LiveTranslationSession
import tv.darshini.app.plugins.StreamVaultPluginManager
import tv.darshini.app.util.isPlaybackComplete
...
import tv.darshini.data.remote.stalker.StalkerUrlFactory
import tv.darshini.data.remote.xtream.XtreamStreamUrlResolver
import tv.darshini.data.security.CredentialDecryptionException
...
import tv.darshini.domain.model.CombinedM3uProfileMember
...
import tv.darshini.domain.model.Favorite
...
import tv.darshini.domain.usecase.MarkAsWatched
import tv.darshini.domain.usecase.ScheduleRecording
import tv.darshini.domain.usecase.ScheduleRecordingCommand
...
import tv.darshini.domain.repository.DownloadManager
import tv.darshini.player.Media3PlayerEngine
...
import tv.darshini.player.PlayerEngine
import tv.darshini.player.PlayerError
import tv.darshini.player.PlayerSubtitleStyle
import tv.darshini.player.timeshift.LiveTimeshiftBackend
import tv.darshini.player.timeshift.LiveTimeshiftState
import tv.darshini.player.timeshift.LiveTimeshiftStatus
import tv.darshini.player.timeshift.TimeshiftConfig
import tv.darshini.player.playback.applyUnsafeTlsBypass
...
class PlayerViewModel @Inject constructor(
    ...
    internal val favoriteRepository: tv.darshini.domain.repository.FavoriteRepository,
    internal val playbackHistoryRepository: PlaybackHistoryRepository,
    internal val providerRepository: tv.darshini.domain.repository.ProviderRepository,
    internal val combinedM3uRepository: CombinedM3uRepository,
    internal val preferencesRepository: tv.darshini.data.preferences.PreferencesRepository,
    ...
    internal val currentChannelFlow = MutableStateFlow<tv.darshini.domain.model.Channel?>(null)
    val currentChannel: StateFlow<tv.darshini.domain.model.Channel?> = currentChannelFlow.asStateFlow()
    ...
    internal val currentChannelFlowList = MutableStateFlow<List<tv.darshini.domain.model.Channel>>(emptyList())
    val currentChannelList: StateFlow<List<tv.darshini.domain.model.Channel>> = currentChannelFlowList.asStateFlow()

    internal val recentChannelsFlow = MutableStateFlow<List<tv.darshini.domain.model.Channel>>(emptyList())
    val recentChannels: StateFlow<List<tv.darshini.domain.model.Channel>> = recentChannelsFlow.asStateFlow()
    ...
    val remoteShortcutPreferences = preferencesRepository.remoteShortcutPreferences
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5_000L), tv.darshini.domain.model.RemoteShortcutPreferences())
    ...
    private var preferredSurfaceMode: tv.darshini.domain.model.PlayerSurfaceMode =
        tv.darshini.domain.model.PlayerSurfaceMode.AUTO
     ...
     */
    internal var channelList: List<tv.darshini.domain.model.Channel> = emptyList()
        set(value) {
            field = value
            rebuildChannelNumberIndex()
        }
    internal var channelNumberIndex: Map<Int, tv.darshini.domain.model.Channel> = emptyMap()
        private set
    ...
    internal fun logRepositoryFailure(operation: String, result: tv.darshini.domain.model.Result<Unit>) {
    ...
    val playerStats: StateFlow<tv.darshini.player.PlayerStats> =
        activeEngineState(tv.darshini.player.PlayerStats()) { it.playerStats }
    val availableAudioTracks: StateFlow<List<tv.darshini.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableAudioTracks }
    }
    val availableSubtitleTracks: StateFlow<List<tv.darshini.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableSubtitleTracks }
    }
    val availableVideoQualities: StateFlow<List<tv.darshini.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
    ...
    internal suspend fun preparePlayer(
        streamInfo: tv.darshini.domain.model.StreamInfo,
        requestVersion: Long,
    ...
    private suspend fun probePlaybackUrl(streamInfo: tv.darshini.domain.model.StreamInfo): PlaybackProbeFailure? {
    ...
    private suspend fun shouldProbePlaybackUrl(url: String): Boolean {
        ...
        if (provider.type != tv.darshini.domain.model.ProviderType.STALKER_PORTAL && cacheKey in probePassedPlaybackKeys) {
            return false
        }
        return (
            provider.type == tv.darshini.domain.model.ProviderType.XTREAM_CODES ||
                provider.type == tv.darshini.domain.model.ProviderType.STALKER_PORTAL
            ) &&
                    ...
                    _playerDiagnostics.update {
                        it.copy(
                            ...
                            providerSourceLabel = when (provider.type) {
                                tv.darshini.domain.model.ProviderType.XTREAM_CODES -> "Xtream Codes"
                                tv.darshini.domain.model.ProviderType.M3U -> "M3U Playlist"
                                tv.darshini.domain.model.ProviderType.STALKER_PORTAL -> "Stalker/MAG Portal"
                                tv.darshini.domain.model.ProviderType.JELLYFIN -> "Jellyfin"
                            }
                        )
                    }
                }
    ...
    internal fun updateChannelDiagnostics(channel: tv.darshini.domain.model.Channel) {
    ...
    internal suspend fun resolvePlaybackStreamInfo(
        ...
        providerId: Long,
        contentType: ContentType
    ): tv.darshini.domain.model.StreamInfo? {
