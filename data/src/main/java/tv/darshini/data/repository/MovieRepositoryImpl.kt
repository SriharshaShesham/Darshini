...
import tv.darshini.data.local.entity.MovieBrowseEntity
...
import tv.darshini.data.sync.ContentCachePolicy
import tv.darshini.data.sync.SyncManager
import tv.darshini.data.util.MoviePresentationSettings
import tv.darshini.data.util.buildPresentedMovies
import tv.darshini.data.util.rankSearchResults
import tv.darshini.data.util.toFtsPrefixQuery
...
import tv.darshini.domain.model.Result.Success
...
class MovieRepositoryImpl @Inject constructor(
    ...
    private suspend fun loadPresentationCandidates(movie: Movie): List<tv.darshini.data.local.entity.MovieEntity> {
    ...
    private suspend fun fetchMovieBrowseResult(query: LibraryBrowseQuery): PagedResult<Movie> {
        ...
        val hasMoreRemote = query.categoryId?.let { categoryId ->
            ...
            when (provider?.type) {
                ProviderType.STALKER_PORTAL -> movieCategoryHydrationDao.get(query.providerId, categoryId)?.let { !it.isComplete } ?: false
                ProviderType.XTREAM_CODES -> categoryDao.getByProviderCategoryAndType(query.providerId, categoryId, ContentType.MOVIE.name)?.isPartialSync ?: false
                else -> false
            }
        } ?: false
    ...
    private fun tv.darshini.data.local.entity.MovieEntity.hasFreshXtreamDetails(now: Long = System.currentTimeMillis()): Boolean {
