...
import tv.darshini.data.local.dao.CombinedM3uProfileDao
import tv.darshini.data.local.dao.CombinedM3uProfileMemberDao
import tv.darshini.data.local.dao.ProviderDao
import tv.darshini.data.local.entity.CombinedM3uProfileEntity
import tv.darshini.data.local.entity.CombinedM3uProfileMemberEntity
            ...
            if (memberProviders.isEmpty()) {
                ...
                combine(categoryFlows) { arrays ->
                    arrays.toList()
                        .map { it as Triple<*, *, *> }
                        .flatMap { triple ->
                            val member = triple.first as tv.darshini.domain.model.CombinedM3uProfileMember
                            val categories = triple.second as List<Category>
                            val hiddenCategoryIds = triple.third as Set<Long>
                            categories
                                .filter {
                                    !it.isVirtual &&
                                        it.id != tv.darshini.domain.repository.ChannelRepository.ALL_CHANNELS_ID &&
