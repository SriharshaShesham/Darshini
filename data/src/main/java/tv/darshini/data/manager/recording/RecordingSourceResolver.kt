package tv.darshini.data.manager.recording

import tv.darshini.data.local.dao.ProviderDao
import tv.darshini.data.remote.xtream.ResolvedStreamUrl
...
class RecordingSourceResolver @Inject constructor(
    ...
    suspend fun resolveLiveSource(
        ...
        val providerLabel = providerDao.getById(providerId)?.let { provider ->
            when {
                provider.type == tv.darshini.domain.model.ProviderType.XTREAM_CODES -> "${provider.name} • Xtream"
                provider.type == tv.darshini.domain.model.ProviderType.M3U -> "${provider.name} • M3U"
                provider.type == tv.darshini.domain.model.ProviderType.STALKER_PORTAL -> "${provider.name} • Stalker"
                else -> provider.name
            }
        }
