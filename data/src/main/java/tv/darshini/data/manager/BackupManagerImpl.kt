package tv.darshini.data.manager
...
import tv.darshini.data.local.dao.RecordingScheduleDao
import tv.darshini.data.local.dao.VirtualGroupDao
import tv.darshini.data.local.entity.ProviderEntity
import tv.darshini.data.local.entity.RecordingScheduleEntity
import tv.darshini.data.mapper.toDomain
import tv.darshini.data.mapper.toEntity
import tv.darshini.data.preferences.ParentalPinBackupData
import tv.darshini.data.preferences.PreferencesRepository
import tv.darshini.data.security.CredentialCrypto
import tv.darshini.domain.manager.BackupData
import tv.darshini.domain.manager.BackupConflictStrategy
import tv.darshini.domain.manager.BackupImportPlan
import tv.darshini.domain.manager.BackupImportResult
import tv.darshini.domain.manager.BackupManager
import tv.darshini.domain.manager.BackupPreview
import tv.darshini.domain.manager.RecordingScheduleImportDisposition
import tv.darshini.domain.manager.RecordingScheduleImportOutcome
import tv.darshini.domain.manager.RecordingScheduleImportSummary
import tv.darshini.domain.manager.ProtectedCategoryBackup
import tv.darshini.domain.manager.RecordingManager
import tv.darshini.domain.manager.ScheduledRecordingBackup
...
class BackupManagerImpl @Inject constructor(
    ...
    override suspend fun exportConfig(uriString: String): tv.darshini.domain.model.Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            } ?: return@withContext tv.darshini.domain.model.Result.error("Failed to open output stream")

            tv.darshini.domain.model.Result.success(Unit)
        } catch (e: Exception) {
            tv.darshini.domain.model.Result.error("Failed to export backup: ${e.message}", e)
        }
    }

    override suspend fun exportProvider(providerId: Long, uriString: String): tv.darshini.domain.model.Result<Unit> = withContext(Dispatchers.IO) {
        try {
            val providerEntity = providerDao.getById(providerId)
                ?: return@withContext tv.darshini.domain.model.Result.error("Provider not found")
            val decryptedPassword = credentialCrypto.decryptIfNeeded(providerEntity.password)
            val provider = providerEntity.toDomain().copy(password = decryptedPassword)
            ...
            tv.darshini.domain.model.Result.error("Failed to export provider: ${e.message}", e)
        }
    }
    ...
    ): tv.darshini.domain.model.Result<BackupImportResult> = withContext(Dispatchers.IO) {
        try {
            val backupData = readBackupData(uriString)
            ...
            tv.darshini.domain.model.Result.success(
                BackupImportResult(
            ...
            tv.darshini.domain.model.Result.error("Failed to import backup: ${e.message}", e)
        }
    }
    ...
    private suspend fun restorePreferences(prefs: Map<String, String>) {
        ...
        prefs["appLandingDestination"]?.takeIf { it.isNotBlank() }?.let { savedDestination ->
            preferencesRepository.setAppLandingDestination(
                tv.darshini.domain.model.AppLandingDestination.fromStorage(savedDestination)
            )
        }
        ...
        prefs["playerDecoderMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val decoderMode = tv.darshini.domain.model.DecoderMode.entries
        ...
        prefs["playerAudioOutputPreference"]?.takeIf { it.isNotBlank() }?.let { savedPreference ->
            val preference = tv.darshini.domain.model.AudioOutputPreference.entries
        ...
        prefs["playerSurfaceMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val surfaceMode = tv.darshini.domain.model.PlayerSurfaceMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (surfaceMode != null) {
        ...
        prefs["playerLiveStreamFormatMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val formatMode = tv.darshini.domain.model.LiveStreamFormatMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (formatMode != null) {
        ...
        (prefs["playerVodHttpProtocolMode"] ?: prefs["playerMovieHttpProtocolMode"])?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val protocolMode = tv.darshini.domain.model.VodHttpProtocolMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
    ...
    private suspend fun restoreSavedLibrary(
        ...
        val categoriesByProviderId = mutableMapOf<Long, List<tv.darshini.domain.model.Category>>()
    ...
    private fun resolveProviderIdMap(
        storedProviders: List<ProviderEntity>,
        backupProviders: List<tv.darshini.domain.model.Provider>
    ...
    private suspend fun restorePlaybackHistory(
        history: List<tv.darshini.domain.model.PlaybackHistory>,
        storedProviders: List<ProviderEntity>,
        backupProviders: List<tv.darshini.domain.model.Provider>,
        conflictStrategy: BackupConflictStrategy
    ) {
...
internal fun tv.darshini.domain.model.RecordingItem.toScheduledRecordingBackup(
    provider: tv.darshini.domain.model.Provider,
...
internal suspend fun importScheduledRecordingBackups(
    ...
    storedProviders: List<ProviderEntity>,
    existingSchedules: MutableList<tv.darshini.domain.model.RecordingItem>,
    conflictStrategy: BackupConflictStrategy,
    recordingManager: RecordingManager,
...
private val FAVORITE_LIST_TYPE: Type = object : TypeToken<List<tv.darshini.domain.model.Favorite>>() {}.type
private val VIRTUAL_GROUP_LIST_TYPE: Type = object : TypeToken<List<tv.darshini.domain.model.VirtualGroup>>() {}.type
private val PLAYBACK_HISTORY_LIST_TYPE: Type = object : TypeToken<List<tv.darshini.domain.model.PlaybackHistory>>() {}.type
