...
internal suspend fun reconcileTargetedProviderStatus(
    ...
    provider: tv.darshini.data.local.entity.ProviderEntity,
    result: tv.darshini.domain.model.Result<Unit>,
    ...
    when (result) {
        ...
        is tv.darshini.domain.model.Result.Loading -> Unit
    }
}
    ...
    private suspend fun syncXtreamProviderIfStale(
        ...
        if (liveStale) {
            when (val liveResult = entryPoint.syncManager().retrySection(
                ...
                is tv.darshini.domain.model.Result.Error -> return liveResult
                else -> Unit
            }
        }
        if (epgStale) {
            when (val epgResult = entryPoint.syncManager().syncEpg(provider.id, force = false)) {
                is tv.darshini.domain.model.Result.Error -> return epgResult
                else -> Unit
            }
        }
        if (movieIndexDue) {
