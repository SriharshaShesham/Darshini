...
import tv.darshini.data.local.dao.CatalogSyncDao
...
import tv.darshini.data.local.dao.MovieCategoryHydrationDao
...
import tv.darshini.data.local.dao.SeriesCategoryHydrationDao
import tv.darshini.data.local.dao.SeriesDao
import tv.darshini.data.local.dao.TmdbIdentityDao
import tv.darshini.data.local.dao.XtreamContentIndexDao
import tv.darshini.data.local.dao.XtreamIndexJobDao
import tv.darshini.data.local.dao.XtreamLiveOnboardingDao
import tv.darshini.data.local.entity.CategoryEntity
import tv.darshini.data.local.entity.ChannelEntity
import tv.darshini.data.local.entity.ChannelGuideSyncEntity
import tv.darshini.data.local.entity.MovieEntity
import tv.darshini.data.local.entity.SeriesEntity
import tv.darshini.data.local.entity.XtreamContentIndexEntity
import tv.darshini.data.local.entity.XtreamIndexJobEntity
import tv.darshini.data.local.entity.XtreamLiveOnboardingStateEntity
import tv.darshini.data.mapper.toDomain
import tv.darshini.data.mapper.toEntity
import tv.darshini.data.parser.M3uParser
import tv.darshini.data.remote.http.buildAppRequestProfile
import tv.darshini.data.remote.http.toGenericRequestProfile
import tv.darshini.data.remote.stalker.StalkerApiService
import tv.darshini.data.remote.stalker.StalkerPlaybackMode
import tv.darshini.data.remote.stalker.StalkerProvider
import tv.darshini.data.remote.stalker.StalkerProviderProfile
import tv.darshini.data.remote.jellyfin.JellyfinProvider
import tv.darshini.data.remote.stalker.StalkerTrafficCoordinator
import tv.darshini.data.remote.dto.XtreamCategory
import tv.darshini.data.remote.dto.XtreamSeriesItem
import tv.darshini.data.remote.dto.XtreamStream
import tv.darshini.data.remote.xtream.OkHttpXtreamApiService
import tv.darshini.data.remote.xtream.XtreamAuthenticationException
import tv.darshini.data.remote.xtream.XtreamNetworkException
import tv.darshini.data.remote.xtream.XtreamParsingException
import tv.darshini.data.remote.xtream.XtreamRequestException
import tv.darshini.data.remote.xtream.XtreamResponseTooLargeException
import tv.darshini.data.remote.xtream.XtreamApiService
import tv.darshini.data.remote.xtream.XtreamProvider
import tv.darshini.data.remote.xtream.XtreamUrlFactory
...
import tv.darshini.domain.model.VodSyncMode
import tv.darshini.domain.repository.EpgRepository
import tv.darshini.domain.repository.EpgSourceRepository
import tv.darshini.domain.repository.SyncMetadataRepository
import tv.darshini.domain.sync.Section
import tv.darshini.domain.sync.SyncProgress
...
private const val PREVIEW_SYNC_ITEM_LIMIT = 20
...
class SyncManager @Inject constructor(
    ...
    ): tv.darshini.domain.model.Result<Unit> {
        val providerEntity = providerDao.getById(providerId)
            ?: return tv.darshini.domain.model.Result.error("Provider $providerId not found")
        ...
        return if (providerEntity.type == ProviderType.STALKER_PORTAL) {
            ...
            withProviderLock(providerId) {
                val freshProviderEntity = providerDao.getById(providerId)
                    ?: return@withProviderLock tv.darshini.domain.model.Result.error("Provider $providerId not found")
    ...
    private suspend fun syncEpgLocked(
        providerEntity: tv.darshini.data.local.entity.ProviderEntity,
        force: Boolean,
        ...
        return try {
            ...
            tv.darshini.domain.model.Result.error(safeMessage, e)
        }
    }
    ...
    ): tv.darshini.domain.model.Result<Unit> = withProviderLock(providerId) lock@{
        try {
            val providerEntity = providerDao.getById(providerId)
                ?: return@lock tv.darshini.domain.model.Result.error("Provider $providerId not found")
    ...
    suspend fun syncSingleCategory(
        providerId: Long,
        ...
        val categoryEntity = categoryDao.getByProviderCategoryAndType(providerId, categoryId, contentType.name)
            ?: return@lock tv.darshini.domain.model.Result.error("Category $categoryId not found")
                ...
                when (provider.type) {
                    ProviderType.XTREAM_CODES -> {
                        ...
                        val outcome = when (contentType) {
                            ContentType.MOVIE -> fetchMovieCategoryOutcome(provider, api, categoryEntity.toXtreamCategory())
                            ContentType.SERIES -> fetchSeriesCategoryOutcome(provider, api, categoryEntity.toXtreamCategory())
                            else -> throw IllegalArgumentException("Unsupported category content type: $contentType")
                        }
                        when (val categoryOutcome = outcome.outcome) {
                            is CategoryFetchOutcome.Success -> {
                                ...
                                categoryDao.updateIsPartialSync(provider.id, categoryId, contentType.name, isPartial = false)
                            }
                            is CategoryFetchOutcome.Failure -> {
                                throw categoryOutcome.error
                            }
                            else -> Unit
                        }
                    }
                    ProviderType.STALKER_PORTAL -> {
                        ...
                        val nextPage = 1
                        val pagedResult = withStalkerFetchPermit(provider) {
                        ...
                        when (pagedResult) {
                            is Result.Success -> {
                                val dedupedItems = dedupeStalkerPageItems(pagedResult.data.items, contentType)
                                val indexedAt = System.currentTimeMillis()
                                when (contentType) {
                                    ContentType.MOVIE -> upsertXtreamMovieSummaryBatch(
                                        provider.id,
                                        dedupedItems.filterIsInstance<Movie>(),
                                        indexedAt,
                                        restoreWatchProgress = true
                                    )
                                    ContentType.SERIES -> upsertXtreamSeriesSummaryBatch(
                                        provider.id,
                                        dedupedItems.filterIsInstance<Series>(),
                                        indexedAt
                                    )
                                    else -> 0
                                }
                                val hydration = getStalkerHydrationSnapshot(provider.id, contentType, categoryId)
                                markStalkerAttemptSucceeded(
                                    ...
                                    itemCount = dedupedItems.size,
                                    totalPages = pagedResult.data.totalPages,
                                    pageSize = pagedResult.data.pageSize,
                                    now = indexedAt,
                                    pageComplete = true,
                                    pageFingerprint = stalkerPageFingerprint(pagedResult.data.items, contentType)
                                )
                            }
                            is Result.Error -> {
                                throw pagedResult.exception ?: IOException(pagedResult.message)
                            }
                            else -> Unit
                        }
                    }
                    else -> throw IllegalArgumentException("Category sync is not supported for provider type ${provider.type}")
                }
            }
            tv.darshini.domain.model.Result.success(Unit)
        } catch (e: Exception) {
            Log.e(TAG, "Category sync failed for provider $providerId, category $categoryId: ${e.message}", e)
            tv.darshini.domain.model.Result.error(e.message ?: "Category sync failed", e)
        }
    }
    ...
    suspend fun rebuildXtreamIndex(
        ...
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock tv.darshini.domain.model.Result.error("Index rebuild is only available for Xtream providers")
        }
    ...
    suspend fun processQueuedXtreamIndexJobs(
        ...
        val providerEntity = providerDao.getById(providerId)
            ?: return@lock tv.darshini.domain.model.Result.error("Provider $providerId not found")
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock tv.darshini.domain.model.Result.success(Unit)
        }
        ...
        if (warnings.isNotEmpty()) {
            ...
            tv.darshini.domain.model.Result.error(warnings.first(), exception)
        } else {
            tv.darshini.domain.model.Result.success(Unit)
        }
    }
    ...
    private suspend fun processStalkerSummaryIndexSection(
        ...
        data class StalkerCategoryFetchResult(
            ...
            val result: Result<tv.darshini.data.remote.stalker.StalkerPagedResult<out Any>>
        )
        ...
        while (pendingQueue.isNotEmpty() && System.currentTimeMillis() < runDeadlineAt) {
            ...
            for (fetched in fetchedResults) {
                ...
                is Result.Success -> {
                    ...
                    val dedupedItems = dedupeStalkerPageItems(result.data.items, contentType).take(PREVIEW_SYNC_ITEM_LIMIT)
            ...
            when (val result = fetchStalkerWildcardSummaryPageWithRecovery(api, contentType, wildcardCategory.categoryId, nextPage)) {
                is Result.Success -> {
                    ...
                    val anomaly = detectStalkerPageAnomaly(
                        hydration = currentHydration,
                        requestedPage = nextPage,
                        pagedResult = tv.darshini.data.remote.stalker.StalkerPagedResult(
                            items = visibleItems,
    ...
    private suspend fun markStalkerAttemptStarted(
        ...
        when (contentType) {
            ContentType.MOVIE -> movieCategoryHydrationDao.upsert(
                tv.darshini.data.local.entity.MovieCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
            ...
            ContentType.SERIES -> seriesCategoryHydrationDao.upsert(
                tv.darshini.data.local.entity.SeriesCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
                    lastHydratedAt = hydration?.lastHydratedAt ?: 0L,
                    itemCount = hydration?.itemCount ?: 0,
    ...
    private suspend fun fetchStalkerSummaryPageWithRecovery(
        ...
        categoryId: Long,
        page: Int
    ): Result<tv.darshini.data.remote.stalker.StalkerPagedResult<out Any>> {
    ...
    private suspend fun fetchStalkerSummaryFirstPageFallback(
        ...
        contentType: ContentType,
        categoryId: Long
    ): Result<tv.darshini.data.remote.stalker.StalkerPagedResult<out Any>> = when (contentType) {
        ContentType.MOVIE -> when (val result = api.getVodStreams(categoryId)) {
            is Result.Success -> Result.success(
                tv.darshini.data.remote.stalker.StalkerPagedResult(
                    items = result.data,
                    page = 1,
                    totalPages = 1,
    ...
    private fun detectStalkerPageAnomaly(
        hydration: StalkerHydrationSnapshot?,
        requestedPage: Int,
        pagedResult: tv.darshini.data.remote.stalker.StalkerPagedResult<out Any>,
        pageFingerprint: String?
    ): String? {
    ...
    private suspend fun processXtreamSummaryIndexSection(
        ...
        val hiddenCategoryIds = preferencesRepository.getHiddenCategoryIds(provider.id, contentType).first()
        ...
        for (workItem in workItems) {
            ...
            if (category.categoryId in hiddenCategoryIds) {
                if (workItem.advancesCursor) completedCategories++
                continue
            }
            ...
            when (val categoryOutcome = outcome.outcome) {
                is CategoryFetchOutcome.Success -> {
                    ...
                    val isPartial = categoryOutcome.rawCount > categoryOutcome.items.size
                    categoryDao.updateIsPartialSync(provider.id, category.categoryId, contentType.name, isPartial)
            ...
            ContentType.MOVIE -> api.streamVodSummaries(adultCategoryIds = adultCategoryIds) { batch ->
                val accepted = upsertXtreamMovieSummaryBatch(
                    ...
                    movies = batch.withMovieCategoryNames(categoryNamesById)
                        .filter { movie -> movie.categoryId == null || movie.categoryId !in hiddenCategoryIds },
                    indexedAt = System.currentTimeMillis(),
                    restoreWatchProgress = false
                )
                indexedRows += accepted
            ...
            ContentType.SERIES -> api.streamSeriesSummaries(adultCategoryIds = adultCategoryIds) { batch ->
                val accepted = upsertXtreamSeriesSummaryBatch(
                    ...
                    series = batch.withSeriesCategoryNames(categoryNamesById)
                        .filter { series -> series.categoryId == null || series.categoryId !in hiddenCategoryIds },
                    indexedAt = System.currentTimeMillis()
                )
                indexedRows += accepted
                upsertXtreamIndexJob(
    ...
    private suspend fun syncStalker(
        ...
        if (AppTopLevelDestination.MOVIES in stalkerEnabledDestinations &&
            ...
            val categories = when (val categoriesResult = api.getVodCategories()) {
                ...
                is tv.darshini.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
            }
            emitCatalogSyncProgress(
                section = Section.VOD,
    ...
    private suspend fun syncJellyfin(
        ...
        try {
            ...
            if (resolvedMovies is tv.darshini.domain.model.Result.Error) android.util.Log.e("JellyfinSync", "Movies error: ${resolvedMovies.message}")
            if (resolvedSeries is tv.darshini.domain.model.Result.Error) android.util.Log.e("JellyfinSync", "Series error: ${resolvedSeries.message}")

            if (resolvedMovies is tv.darshini.domain.model.Result.Error && resolvedSeries is tv.darshini.domain.model.Result.Error) {
            ...
            val movieEntities = (resolvedMovies as? tv.darshini.domain.model.Result.Success)?.data.orEmpty()
            val seriesEntities = (resolvedSeries as? tv.darshini.domain.model.Result.Success)?.data.orEmpty()
            ...
            if (movieEntities.isNotEmpty()) {
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, tv.darshini.domain.model.ContentType.MOVIE.name, listOf(
                        tv.darshini.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 1L, name = "Movies",
                            type = tv.darshini.domain.model.ContentType.MOVIE
                        )
                    ))
                }
            }
            ...
            if (seriesEntities.isNotEmpty()) {
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, tv.darshini.domain.model.ContentType.SERIES.name, listOf(
                        tv.darshini.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 2L, name = "Series",
                            type = tv.darshini.domain.model.ContentType.SERIES
                        )
                    ))
                }
            }
    ...
    private suspend fun syncStalkerPortalEpg(
        ...
        val insertBuffer = ArrayList<tv.darshini.data.local.entity.ProgramEntity>(STALKER_GUIDE_PROGRAM_BATCH_SIZE)
        ...
        runCatching {
            ...
            api.streamBulkEpg(periodHours = 6) { program ->
                ...
                if (result is tv.darshini.domain.model.Result.Error) {
        ...
        fallbackGuideRequests.forEachIndexed { index, request ->
            ...
            runCatching {
                ...
                if (streamResult is tv.darshini.domain.model.Result.Error) {
    ...
    private suspend fun syncXtreamLiveCatalog(
        ...
        syncProgressBus.emit(
            tv.darshini.domain.sync.SyncProgress(
                section = tv.darshini.domain.sync.Section.LIVE,
                current = 0,
                total = 0,
                currentLabel = "",
                itemsIndexed = 0
    ...
    private suspend fun loadStalkerChannelsByCategory(
        api: StalkerProvider,
        categories: List<tv.darshini.domain.model.Category>,
        onProgress: ((String) -> Unit)?
    ...
    private suspend fun syncStalkerLiveCatalogStaged(
        ...
        try {
            ...
            when (val streamResult = api.streamLiveStreams { channel ->
                ...
                is tv.darshini.domain.model.Result.Success -> {
                    flushBatch()
    ...
    private suspend fun stalkerCatalogAccessDiagnostic(
        ...
        val profile = when (val profileResult = api.getAccountProfile()) {
            is tv.darshini.domain.model.Result.Success -> profileResult.data
            else -> return null
        }
    ...
    private fun <T> requireResult(result: tv.darshini.domain.model.Result<T>, fallbackMessage: String): T {
        return when (result) {
            is tv.darshini.domain.model.Result.Success -> result.data
            is tv.darshini.domain.model.Result.Error -> throw IllegalStateException(result.message.ifBlank { fallbackMessage }, result.exception)
            is tv.darshini.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
        }
    }
    ...
    ): TimedCategoryOutcome<Movie> = xtreamFetcher.fetchMovieCategoryOutcome(provider, api, category, previewLimit)
    ...
    private suspend fun fetchSeriesCategoryOutcome(
        ...
        category: XtreamCategory,
        previewLimit: Int? = null
    ): TimedCategoryOutcome<Series> = xtreamFetcher.fetchSeriesCategoryOutcome(provider, api, category, previewLimit)
