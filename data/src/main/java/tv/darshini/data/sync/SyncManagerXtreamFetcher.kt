    ...
    suspend fun fetchMovieCategoryOutcome(
        ...
        val outcome = when {
            ...
            rawStreams.isEmpty() -> {
                ...
                CategoryFetchOutcome.Success(category.categoryName, api.mapVodStreamsResponse(rawStreams).let { if (previewLimit != null) it.take(previewLimit) else it }, rawStreams.size)
            }
        }
        return TimedCategoryOutcome(category, outcome, elapsedMs)
    }
    ...
    suspend fun fetchSeriesCategoryOutcome(
        ...
        val outcome = when {
            ...
            rawSeries.isEmpty() -> {
                ...
                CategoryFetchOutcome.Success(category.categoryName, api.mapSeriesListResponse(rawSeries).let { if (previewLimit != null) it.take(previewLimit) else it }, rawSeries.size)
            }
        }
        return TimedCategoryOutcome(category, outcome, elapsedMs)
    }
