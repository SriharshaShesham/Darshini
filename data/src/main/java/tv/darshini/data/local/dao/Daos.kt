...
interface CategoryDao {
    ...
    @Query("SELECT * FROM categories WHERE provider_id = :providerId AND category_id = :categoryId AND type = :type LIMIT 1")
    suspend fun getByProviderCategoryAndType(providerId: Long, categoryId: Long, type: String): CategoryEntity?

    @Query("UPDATE categories SET is_partial_sync = :isPartial WHERE provider_id = :providerId AND category_id = :categoryId AND type = :type")
    suspend fun updateIsPartialSync(providerId: Long, categoryId: Long, type: String, isPartial: Boolean)
}
