# Darshini (StreamVault-IPTV)

**Project**: Android TV IPTV player app with support for multiple provider types (Xtream Codes, Stalker Portal, Jellyfin, M3U).
**Tech Stack**: Kotlin + Jetpack Compose, Media3 (ExoPlayer), Room, Hilt, Retrofit
**Min Android**: API 25 (Android 7.1)
**Target**: TV-first UX with D-pad navigation; phone/tablet support secondary

## Build System & Module Structure

- **Build Tool**: Gradle with Kotlin DSL (`build.gradle.kts` files)
- **Modules**: 
  - `app` — Main Android app with UI, Activities, Composables
  - `domain` — Entities, use cases, business logic
  - `data` — Repositories, database (Room), network (Retrofit)
  - `player` — Media3 player wrapper with decoder fallback logic

**Build commands**:
```bash
./gradlew build              # Full build
./gradlew app:assembleDebug  # Debug APK
./gradlew app:installDebug   # Install to emulator/device
...
```bash
adb shell dumpsys media_session | awk '/package=tv.darshini.app/{seen=1} seen && /metadata:/{print; getline; print; getline; print} seen && /state=PlaybackState/{print; exit}'
